  ...
  <v-app>
    ...
    <v-row v-if="$store.getters.loaded != 'completed'" class="fill-height" align-content="center" justify="center">
      <v-col cols="12" class="text-center">
        <v-icon x-large :color="$store.getters.getFromLoadingStatus.color">{{
          $store.getters.getFromLoadingStatus.mdi
        }}</v-icon>
      </v-col>
      ...
      <v-col cols="6">
        <v-progress-linear
          :color="$store.getters.getFromLoadingStatus.color"
          indeterminate
          rounded
          height="6"
        ></v-progress-linear>
    ...
    <v-snackbar
      ...
      elevation="20"
      class="primary"
      :class="'text-'+$store.getters.getAlert.type"
      v-model="$store.getters.getAlert.show"
      timeout="2000"
    >
      <template>
        <v-icon class="mb-1" :color="$store.getters.getAlert.type"
          >mdi {{ $store.getters.getAlert.mdi }}</v-icon
        >
...
export default {
  async mounted() {
    ...
    if (
      !this.$store.getters.getUser.isAuth &&
